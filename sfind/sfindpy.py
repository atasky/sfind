...
def SFINDPY(domain, dns=None, useragent=None, timeout=None, threads=None, recon=None, bruteforce=None, wordlist=None):
    def sfindpy(domain, dns=None, useragent=None, timeout=None):
    ...
    if isinstance(domain, list):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
            futures = [executor.submit(sfindpy, d, dns, useragent, timeout) for d in domain]
    ...
    return sfindpy(domain, dns=None, useragent=None, timeout=None)
...
def main():
    parser = argparse.ArgumentParser(
        prog="SFINDPY", 
        description=f"sfindpy v.{__version__} - Subdomain Scan\nhttps://github.com/atasky/sfind", 
    ...
    if not args.domain and not args.file:
        ...
        if not sys.stdin.isatty():
            ...
            if len(stdin) == 1:
                # check if is file
                # echo "/path/to/domains.txt" | sfindpy
                if os.path.isfile(stdin[0]):
                    args.file = stdin[0]
                # otherwise domain is passed
                # echo "domain.com" | sfindpy
                else:
                    args.domain = stdin[0]
            # domains list via file
            # cat domains.txt | sfindpy
            elif len(stdin) > 1:
                args.domain = stdin
        elif args.report:
    ...
    if args.domain:
        ...
        results = SFINDPY(domain, args.dns, args.useragent, args.timeout)
    ...
    if args.file:
        ...
        results = SFINDPY(domains, args.dns, args.useragent, args.timeout)
        output(results, args.json)
